...
fn eocd_cd_lf_chain_read_test() -> anyhow::Result<()> {
  ...
  assert_eq!(local_file_header.compressed_size as usize, expected_content.len());
  assert_eq!(local_file_header.uncompressed_size as usize, expected_content.len());
  assert_eq!(String::from_utf8_lossy(&local_file_header.compressed_data), expected_content);
